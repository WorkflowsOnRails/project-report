...
This appendix contains the detailed use cases for the fourth year project system that was developed as a case study in chapter \ref{ch:case-study-1}. Section \ref{sec:detailed-use-cases-1-setup} contains use cases related to configuring the system for a new year and new slate of students. The use cases in section \ref{sec:account-management} cover details of creating, modifying, and deleting user accounts, while those in section \ref{sec:detailed-use-cases-1-management} describe the activities that culminate in students selecting a project. Finally, section \ref{sec:detailed-use-cases-1-execution} contains descriptions of the use cases involved in carrying out the project, such as submitting proposals and other deliverables.
...
\begin{table}[!htbp]
  ...
  \begin{usecase}[Schedule Oral Presentations]
    \ucpart{Description}
    The Project Coordinator determines when and where each group will have their oral presentation, and informs the groups.
    %
    \ucpart{Actors}
    Project Coordinator
    %
    ...
    \ucnormal
    \begin{ucenum}
      \item The Project Coordinator opens the oral presentation scheduling interface.
      \item The Project Coordinator inputs the week to schedule presentations for, plus the earliest starting time and latest end time that a presentation may have.
      \item The Project Coordinator submits the scheduling request to the system.
      \item The system assigns a timeslot to each project group, and informs the Project Coordinator when scheduling has been completed.
      \item The Project Coordinator notifies the project groups that the presentation schedule has been created.
    ...
    If a Project Group Member or Project Supervisor is no longer available for the timeslot they were assigned, they must contact the Project Coordinator. The Project Coordinator may update the scheduling forms and repeat the process from step 1 with the new scheduling constraints.
    %
    \ucpart{Postconditions}
